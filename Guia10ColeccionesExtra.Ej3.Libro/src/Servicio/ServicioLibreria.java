...
public class ServicioLibreria {
    ...
    public ServicioLibreria(Set<Libro> libros) {
        this.libros = libros;
    }
    ...
    public boolean prestamo(String titulo) {
        for (Libro libro : libros) {
            if (libro.getTitulo().equalsIgnoreCase(titulo)) {
                if (libro.getNumeroejemplares() > libro.getNumerodeejemplaresprestados()) {
        ...
        return false; // El libro no se encuentra en la biblioteca
    }

    public boolean devolucion(String tituloDevolucion) {
        for (Libro libro : libros) {
            if (libro.getTitulo().equalsIgnoreCase(tituloDevolucion)) {
                if (libro.getNumerodeejemplaresprestados() > 0) {
                    libro.setNumeroejemplares(libro.getNumerodeejemplaresprestados() - 1);
                    return true;
                } else {
                    return false; // No hay ejemplares prestados para devolver
                }
            }
        }
        return false; // El libro no se encuentra en la biblioteca
